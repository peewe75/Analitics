...
const API_BASE = import.meta.env.VITE_API_URL ? `${import.meta.env.VITE_API_URL}/admin` : "http://localhost:8000/api/v1/admin";
const PAYMENTS_API_BASE = import.meta.env.VITE_API_URL ? `${import.meta.env.VITE_API_URL}/payments` : "http://localhost:8000/api/v1/payments";
...
export default function App() {
    ...
    const fetchStats = () => fetch(`${API_BASE}/stats`).then(r => r.json()).then(setStats).catch(() => { });
    const fetchUsers = () => fetch(`${API_BASE}/users`).then(r => r.json()).then(setUsers).catch(() => { });
    const fetchPayments = () => fetch(`${API_BASE}/payments/pending`).then(r => r.json()).then(setPayments).catch(() => { });

    const verifyPayment = (id: number) => {
        fetch(`${PAYMENTS_API_BASE}/${id}/verify`, { method: 'POST' })
        ...
        <div className="admin-layout">
            <aside className="sidebar">
                ...
                <div className="user-profile-bottom">
                    ...
                    <button style={{ background: 'transparent', border: 'none', padding: 0, marginLeft: 'auto', color: 'var(--text-tertiary)' }} onClick={() => signOut()}>
                        <LogOut size={18} />
                    </button>
                </div>
            </aside>
