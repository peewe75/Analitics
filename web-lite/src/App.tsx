...
export default function App() {
    ...
    const fetchAffiliateData = () => {
        setAffLoading(true);
        fetch(`${API_BASE}/affiliates/dashboard?user_id=${MOCK_USER_ID}`)
            .then(r => {
                if (r.status === 404) {
    ...
    const joinAffiliateProgram = () => {
        setAffLoading(true);
        fetch(`${API_BASE}/affiliates/join?user_id=${MOCK_USER_ID}`, {
            method: 'POST'
        })
            .then(r => r.json())
            .then(data => {
    ...
    const handleAcceptConsent = () => {
        fetch(`${API_BASE}/consent/acknowledge`, {
            ...
            .then(r => r.json())
            .then(data => {
                if (data.status === 'success') setHasConsent(true);
            });
    };
    ...
    const runAnalysis = () => {
        ...
        fetch(`${API_BASE}/analyze?symbol=${selectedAsset}&user_id=${MOCK_USER_ID}`)
            .then(r => r.json())
            .then(data => {
    ...
    const submitPayment = (method: string) => {
        ...
        fetch(`${API_BASE}/payments/submit`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
                ...
                <div className="container fade-in-up">
                    ...
                    {activeTab === 'analysis' && (
                        <div className="view-analysis">
                            {/* Analysis view content code stays the same... */}
                    ...
                    {activeTab === 'subscription' && (
                        <div className="view-subscription">
                            {/* Subscription view content code stays the same... */}
